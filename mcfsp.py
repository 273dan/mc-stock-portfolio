...
def getCovReturns(days):
    ...
    for i, row in enumerate(st.session_state['portfolio_data']):
        ...
        returns_df = pd.concat(
            (returns_df, row.pct_change()), ignore_index=True, axis=1)
    ...
    random_norm = np.random.normal(
        size=(days, len(st.session_state['portfolio'])))
...
def showGraph(data):
    ...
    with st.empty():
        ...
        while line < data.shape[0] + 1:
            ...
            st.line_chart(anim_data.T, x_label='Day',
...
with stock_entry:
    stocks_plain = st.text_input(
        'Enter tickers separated by spaces', placeholder='AAPL ^GSPC ^FTSE')
...
if st.session_state['stocks_valid']:
    ...
    with stock_weight:
        weights = [st.slider(f'Weight: {stock}', min_value=0, max_value=100, value=100 // len(
            st.session_state['portfolio'])) for stock in st.session_state['portfolio']]
...
if st.session_state['weights_valid'] and st.session_state['stocks_valid']:
    ...
    with sim_prefs:
        ...
        n_sims = st.slider('Number of simulations to run',
                           min_value=5, max_value=500, value=100)
        n_days = st.slider('Number of days ahead to simulate',
                           min_value=100, max_value=1000, value=365)
        pf_val = st.number_input(
            'Initial portfolio value', min_value=0.0, step=1000.0, value=1000.0)
...
if st.session_state['show_results']:
    ...
    with av:
        av_mtr = st.metric(f'Mean portfolio value after day {
                           n_days}', f'£{average_pfval}', f'{av_return}%')
    with mnmx:
        max_mtr = st.metric(f'Maximum portfolio value', f'£{
                            max_pfval}', f'{max_return}%')
        min_mtr = st.metric(f'Minimum portfolio value', f'£{
                            min_pfval}', f'{min_return}%')

    with vol:
        std_pfval = round(vals.std(), 2)
        std_return = round(returns.std(), 2)

        std_mtr = st.metric(f'Volatility (standard deviation)',
                            std_pfval, std_return, delta_color='off')
    ...
    with qs:
        ...
        med_mtr = st.metric(f'Median portfolio value', f'£{
                            med_val}', f'{med_return}%')
        uq_mtr = st.metric(f'Upper quartile portfolio value',
                           f'£{uq_val}', f'{uq_return}$')
        lq_mtr = st.metric(f'Lower quartile portfolio value',
                           f'£{lq_val}', f'{lq_return}%')

    st.subheader('Risk')
    v90, v95, v99 = st.columns([1/3, 1/3, 1/3])

    # Compute and display value at risk for 3 different confidence intervals
    var90 = round(pf_val - np.percentile(vals, 10), 2)
    var_return90 = round(np.percentile(returns, 10), 2)

    var95 = round(pf_val - np.percentile(vals, 5), 2)
    var_return95 = round(np.percentile(returns, 5), 2)

    var99 = round(pf_val - np.percentile(vals, 1), 2)
